...
import "./Culture.css";
const Century = (props) => {
  const { centuryName } = useParams();
  const [combinedData, setCombinedData] = useState([]);

  useEffect(() => {
    async function centuryData() {
      try {
        const response = await axios.get(
          `https://api.harvardartmuseums.org/object?century=${centuryName}&apikey=8d1c8a53-95e9-43d2-aee5-2bf8403468ce`
        );
    ...
    centuryData();
  }, [centuryName]); // Run effect whenever culturename changes
